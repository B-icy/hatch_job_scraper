...
def find_jobs_from(website, job_title, location, desired_characs, filename="scraped_data.json"):
    """
    ...
    save_jobs_to_json(jobs_list, filename)
...
def save_jobs_to_json(jobs_list, filename):
    jobs = pd.DataFrame(jobs_list)
    jobs.to_json(filename)
    print(jobs_list)
...
def load_indeed_jobs_div(job_title, location):
    getVars = {'q' : job_title, 'l' : location, 'fromage' : 'last','limit' : 50, 'sort' : 'date'}
...
def extract_job_information_indeed(job_soup, desired_characs):
    ...
    if 'summary' in desired_characs:
        ...
        cols.append('summary')
        for job_elem in job_elems:
            summ.append(extract_summary_indeed(job_elem))
        extracted_info.append(summ)
...
def extract_summary_indeed(job_elem):
    ...
    summary_elem = job_elem.find(class_='summary')
